class Solution {
    ...
    private Triplet minFallingPathSum(int row, int[][] grid){
        ...
        for(int col=0;col<grid[0].length;col++){
            int sum=grid[row][col]+((col != nextRowTriplet.minSumIdx) ? nextRowTriplet.minSum : nextRowTriplet.secondMinSum);
            if(sum <= currentTriplet.minSum){
                currentTriplet.secondMinSum = currentTriplet.minSum;
                currentTriplet.minSum = sum;
                currentTriplet.minSumIdx = col;
...
class Triplet{
    ...
    int secondMinSum;
    int minSumIdx;
    Triplet(int minSum, int secondMinSum, int minSumIdx){
        this.minSum=minSum;
        this.secondMinSum=secondMinSum;
        this.minSumIdx=minSumIdx;
    }
}